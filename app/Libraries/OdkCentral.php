...
class OdkCentral
{
    ...
    public function create($file = null, $publish = false, $ignoreWarnings = false)
    {
        ...
        if(is_object($file)) {
            ...
            $this->headers = [
                ...
                'X-XlsForm-FormId-Fallback' => urlencode($file->getClientName()),
            ];
	...
	public function xmlUpdate($form)
    {
		$this->headers = [
            'Content-Type' => 'application/xml',
        ];
    ...
    */
    ...
    public function odata($url= '', $top = false, $skip = false, $count = false, $wkt = false, $filter = '', $expand = false)
    {
        ...
        if($skip){
            $parameter['%24skip']=$skip;
        }
        if($count){
            $parameter['%24count']='true';
        }
        if($wkt){
            $parameter['%24wkt']='true';
        }
        if($filter){
            $parameter['%24filter']=$filter;
        }
        if($expand){
            $parameter['%24filter']=$expand;
        }
        $purl='';
        ...
        $this->endpoint = $base . '.svc/' . $url . '?'.$purl;
       // echo $this->endpoint;
        $this->params = [];
    ...
    public function pdata($url= '',$parameter=[])
    {
        ...
        $purl='';
        if(isset($parameter['top'])){
            $purl.='&%24top='.$parameter['top'];
        }
        if(isset($parameter['skip'])){
            $purl.='&%24skip='.$parameter['skip'];
        }
        if(isset($parameter['count'])){
            $purl.='&%24count=true';
        }
        if(isset($parameter['wkt'])){
            $purl.='&%24wkt=true';
        }
        if(isset($parameter['filter'])){
            $purl.='&%24filter='.$parameter['filter'];
        }
        if(isset($parameter['expand'])){
            $purl.='&%24expand='.$parameter['expand'];
        }
    ...
    public function fetchRepeatTable($data)
    {
        ...
        foreach($data as $key => $value)
        {
            ...
            if (is_array($value))
                ...
                if (strpos($key, "@odata.navigationLink")) {
