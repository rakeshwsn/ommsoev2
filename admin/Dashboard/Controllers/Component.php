...
class Component extends AdminController{
	...
	public function index() {
        ...
        $this->template->set_meta_title(lang('Component.heading_title'));

        if (($this->request->getMethod(1) === 'POST') && $this->validate()) {
        ...
        $data['breadcrumbs'][] = array(
            'text' => lang('Component.heading_title'),
            'href' => admin_url('component')
        );
        ...
        $data['action'] = admin_url('dashboard/component');
        $data['cancel'] = admin_url('extension/dashboard');
