...
FROM soe_transactions t
  ...
  LEFT JOIN soe_districts d
    ON d.id = t.district_id
WHERE t.deleted_at IS NULL AND (b.is_program=1 OR b.is_program IS NULL)";
...
FROM (SELECT
  ...
  LEFT JOIN (SELECT
    ...
    (SELECT
      ...
      WHERE scb.deleted_at IS NULL AND (sb.is_program=1 OR sb.is_program is null)
      AND scb.year > 0
      AND scb.month > 0)
    UNION
      (SELECT
...
WHERE ms.deleted_at IS NULL AND (sb.is_program=1 OR sb.is_program is null)
AND ms.year > 0
AND ms.month > 0)
      ) sts
