...
class IncentiveModel extends Model
{
    ...
    public function phaseDataCheck($data)
    {
       
        $builder = $this->db->table('incetive_main_details');
        $builder->where('district_id', $data['filter_district']);
        $builder->where('block_id', $data['filter_block']);
        $builder->where('year', $data['filter_year']);
        $builder->where('season', $data['filter_season']);
