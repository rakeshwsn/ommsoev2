...
class ClosingBalance extends AdminController {
    ...
    private function validateUpload($year,$month,&$data){
        ...
        if(isset($pending_transactions->total)) {
            ...
            if(isset($pending_transactions['block_pending_uploads'])){
                foreach ($pending_transactions['block_pending_uploads'] as $block_cb) {
            ...
            if(isset($pending_transactions['district_pending_uploads']) && $pending_transactions['district_pending_uploads'] < ($month - 1)){
                $data['error'] = 'Cannot add closing balance. District expenses are not uploaded in the previous months!!';
                $data['can_edit'] = false;
            }
            if(isset($pending_transactions['pending_approvals']) && $pending_transactions['pending_approvals']){
                $data['error'] = 'Cannot add closing balance. Block uploads are not approved';
                $data['can_edit'] = false;
            }
        }
    }
