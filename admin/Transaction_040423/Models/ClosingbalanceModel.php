...
GROUP BY scb.block_id";
            ...
            $data['block_pending_uploads'] = $this->db->query($sql)->getResult();

            //check if pending upload at ATMA
            $sql = "SELECT
  COUNT(id) total
FROM soe_transactions scb
WHERE scb.deleted_at IS NULL
...
AND scb.month < $month";
            ...
            $data['district_pending_uploads'] = $this->db->query($sql)->getFirstRow()->total;
...
AND scb.month <= $month GROUP BY scb.block_id";
            ...
            $data['block_pending_approvals'] = $this->db->query($sql)->getResult();
