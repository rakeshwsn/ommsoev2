...
class SettingModel extends Model
{
	...
	public function editSetting($module, $data)
	{
		...
		$builder->where("module", $module)->delete();
		...
		foreach ($data as $key => $value) {
			if (substr($key, 0, strlen($module)) == $module) {
				if (!is_array($value)) {
					...
					$this->db->table("config")->insert(array("key" => $key, "value" => $value, "module" => $module));
				} else {
					$this->db->table("config")->insert(array("key" => $key, "value" => json_encode(array_values($value), true), "module" => $module, "serialized" => 1));
