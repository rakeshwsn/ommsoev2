...
class Users extends AdminController
{
	...
	public function linkCentral()
	{
		$appuser = $this->userModel->getCentralUsers();
		foreach ($appuser as $user) {
			$data = array(
				'central_appuser_id' => $user['id'],
				'central_appuser_token' => $user['token']
			);
			$username = strtolower($user['displayName']);
			$this->userModel->updateCentralUser($username, $data);
		}
		/*if($appuser){
			$data=$this->request->getPost();
			$data['central_appuser_id']=$appuser['id'];
